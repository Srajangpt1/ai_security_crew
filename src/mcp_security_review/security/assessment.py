...
from .guidelines import SecurityGuidelinesLoader, SecurityGuideline
...
class SecurityAssessment:
    ...
    def _filter_guidelines_by_categories(
        self, 
        guidelines: List[SecurityGuideline], 
        categories: set
    ...
    def _generate_prompt_injection(
        ...
        if security_context.risk_level in ["critical", "high"]:
            injection_parts.append(f"⚠️  HIGH SECURITY RISK DETECTED ({security_context.risk_level.upper()})")
            injection_parts.append("")
    ...
    def _generate_summary(
        ...
        summary_parts.append(f"Security Risk Level: {security_context.risk_level.upper()}")
        
        if security_context.technologies:
            summary_parts.append(f"Technologies: {', '.join(security_context.technologies)}")
        
        if security_context.security_categories:
            categories = [cat.replace("_", " ").title() for cat in security_context.security_categories]
            summary_parts.append(f"Security Categories: {', '.join(categories)}")
        
        if security_context.sensitive_data_types:
            summary_parts.append(f"Sensitive Data Types: {', '.join(security_context.sensitive_data_types)}")
        
        if security_context.attack_vectors:
            vectors = [vec.replace("_", " ").title() for vec in security_context.attack_vectors]
