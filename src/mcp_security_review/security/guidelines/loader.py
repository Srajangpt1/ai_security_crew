...
class SecurityGuidelinesLoader:
    ...
    def _load_all_guidelines(self) -> None:
        ...
        if not self.guidelines_dir.exists():
            logger.warning(
                f"Guidelines directory does not exist: {self.guidelines_dir}"
            )
            return
    ...
    def _parse_guideline_from_content(
        self, content: str, filename: str
    ) -> SecurityGuideline | None:
    ...
    def _extract_category(self, content: str) -> str:
        ...
        if any(
            ...
            keyword in content_lower
            for keyword in ["sql", "database", "query", "injection"]
        ):
            return "database"
        elif any(
            keyword in content_lower
            ...
            keyword in content_lower for keyword in ["encrypt", "hash", "crypto", "ssl"]
        ):
            return "cryptography"
        elif any(
            keyword in content_lower for keyword in ["xss", "csrf", "web", "http"]
        ):
            return "web_security"
        else:
            return "general"
    ...
    def _extract_priority(self, content: str) -> str:
        ...
        priority_match = re.search(
            r"^priority:\s*(.+)$", content, re.MULTILINE | re.IGNORECASE
        )
        if priority_match:
        ...
        content_lower = content.lower()
        if any(
            keyword in content_lower for keyword in ["critical", "urgent", "immediate"]
        ):
            return "critical"
        elif any(
            keyword in content_lower for keyword in ["high", "important", "essential"]
        ):
            return "high"
        elif any(
            keyword in content_lower for keyword in ["low", "optional", "nice to have"]
        ):
            return "low"
        else:
            return "medium"
    ...
    def _extract_description(self, content: str) -> str:
        ...
        desc_match = re.search(
            r"^description:\s*(.+)$", content, re.MULTILINE | re.IGNORECASE
        )
        if desc_match:
        ...
        for paragraph in paragraphs:
            ...
            if (
                ...
                and not paragraph.startswith("title:")
    ...
    def _extract_examples(self, content: str) -> list[str]:
        ...
        examples.extend(
            [f"Code example: {block[:100]}..." for block in code_blocks[:3]]
        )
    ...
    def get_guidelines_for_context(
        self, context: str, technologies: list[str] = None
    ...
    def get_all_categories(self) -> list[str]:
        ...
        categories = {g.category for g in self._guidelines_cache}
    ...
    def search_guidelines(self, query: str) -> list[SecurityGuideline]:
        ...
        for guideline in self._guidelines_cache:
            if (
                ...
                or query_lower in guideline.description.lower()
                or any(query_lower in tag.lower() for tag in guideline.tags)
            ):
    ...
    def _is_relevant_to_context(
        self, guideline: SecurityGuideline, context: str, technologies: list[str] = None
    ) -> bool:
        ...
        if technologies:
            for tech in technologies:
                ...
                if (
                    tech_lower in context_lower
                    or tech_lower in guideline.description.lower()
                    or any(tech_lower in tag.lower() for tag in guideline.tags)
                ):
                    return True
