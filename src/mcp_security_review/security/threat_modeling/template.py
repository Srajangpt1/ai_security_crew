...
class ThreatModelOutput:
    ...
    def to_markdown(self) -> str:
        ...
        for threat in self.threats:
            ...
            lines.append(
                f"- **Data Affected:** {', '.join(threat.data_affected)}"
            )
            ...
            if threat.references:
                ...
                for ref in threat.references:
                    ...
                    lines.append(
                        f"- {ref_label} `{ref.location}` — {ref.description}"
                    )
                    if ref.snippet:
                        lines.append("  ```")
        ...
        if self.references_used:
            ...
            lines.append("")
            lines.append(
                "Previous threat models and documents used as reference:"
            )
            lines.append("")
            for ref_item in self.references_used:
                ...
                if ref_url:
                    lines.append(
                        f"- [{ref_title}]({ref_url}) ({ref_source})"
                    )
                else:
        ...
        lines.append("---")
        lines.append(
            "*Generated by mcp-security-review threat modeling tool*"
        )
        lines.append("")
            ...
            "sections": {
                ...
                "threats": {
                    ...
                    "fields": {
                        ...
                        "mitigation": (
                            "What we're doing or should do about it"
                        ),
                        "status": (
                            "open, in_progress, mitigated, or accepted"
                        ),
                        "references": {
                            "description": (
                                "Evidence linking this threat "
                                "to code or artifacts"
                            ),
                            "fields": {
                                "type": (
                                    "code, artifact, ticket, "
                                    "confluence, or url"
                                ),
                                "location": (
                                    "File path, URL, or ticket key"
                                ),
                                "description": (
                                    "What this reference proves"
                                ),
                                "snippet": (
                                    "Optional inline code/text evidence"
                                ),
                            },
                        },
                        "accepted_risk": (
