...
@jira_mcp.tool(tags={"security", "review", "lightweight"})
async def lightweight_security_review(
    ...
    """
    try:
        # Create a synthetic ticket-like structure for the security assessment
        synthetic_ticket = {
        ...
        # Perform security assessment using the same engine
        security_assessment = SecurityAssessment()
        requirements = security_assessment.assess_ticket(synthetic_ticket)
        
        # Build response with emphasis on pre-coding guidance
        response = {
            "success": True,
        ...
        # Add prompt injection with pre-coding emphasis if requested
        if include_prompt_injection:
        ...
        # Provide fallback security guidance even on error
        fallback_response = {
            "success": False,
            "task_description": task_description,
            "error": error_message,
