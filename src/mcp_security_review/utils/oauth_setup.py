"""
OAuth 2.0 Authorization Flow Helper for MCP Security Review
...
4. Saves the tokens securely for later use by MCP Security Review
"""
...
# Configure logging
logger = logging.getLogger("mcp-security-review.oauth-setup")
...
def run_oauth_flow(args: OAuthSetupArgs) -> bool:
    ...
    if oauth_config.exchange_code_for_tokens(authorization_code):
        ...
        if oauth_config.cloud_id:
            ...
            logger.info(
                f"Token storage location (backup): ~/.mcp-security-review/oauth-{oauth_config.client_id}.json"
            )
...
def run_oauth_setup() -> int:
    ...
    print(
        "This wizard will guide you through setting up OAuth 2.0 authentication for MCP Security Review."
    )
