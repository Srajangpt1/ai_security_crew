...
# Configure logging
logger = logging.getLogger("mcp-security-review.oauth")
...
KEYRING_SERVICE_NAME = "mcp-security-review-oauth"
...
class OAuthConfig:
    ...
    def _save_tokens_to_file(self, token_data: dict = None) -> None:
        ...
        try:
            ...
            token_dir = Path.home() / ".mcp-security-review"
    ...
    def _load_tokens_from_file(client_id: str) -> dict[str, Any]:
        ...
        Returns:
            Dict with the token data or empty dict if no tokens found
        """
        token_path = Path.home() / ".mcp-security-review" / f"oauth-{client_id}.json"
