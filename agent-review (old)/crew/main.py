...
class SecurityCrew:
    ...
    def run(self):
        ...
        review_code_task = tasks.review_code(code_review_agent, self.code_snippet)

        exploit_vulnerability_task = tasks.vulnerability_exploit(
            exploit_vulnerability, context=[review_code_task], code=self.code_snippet
        )

        suggest_mitigation_task = tasks.mitigation_strategy(
            suggest_mitigation,
            context=[review_code_task, exploit_vulnerability_task],
            code=self.code_snippet,
        )

        vulnerability_writeup = tasks.vulnerability_writeup(
            vulnerability_write_up,
            context=[
                review_code_task,
                exploit_vulnerability_task,
                suggest_mitigation_task,
            ],
            code=self.code_snippet,
        )
